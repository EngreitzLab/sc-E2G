...
def statistic_delta_precision_at_threshold(y_true, y_pred_full, y_pred_ablated, thresh_full, thresh_ablated): # return precision_ablated-precision_full
    precision_full, recall_full, thresholds_full = precision_recall_curve_modified(y_true, y_pred_full)
    if thresh_full != None:
    ...
    if thresh_ablated != None:
        idx_threshold_ablated = np.argsort(np.abs(thresholds_ablated - thresh_ablated))[0]
...
def threshold_at_target_recall(y_true, y_pred_full, recall_target):
    precision_full, recall_full, thresholds_full = precision_recall_curve_modified(y_true, y_pred_full)
    if np.max(recall_full) > recall_target:
        idx_recall_at_target = np.argsort(np.abs(recall_full - recall_target))[0] # find index of recall closest to 0.7
        thresh = thresholds_full[idx_recall_at_target+1] #adjust for removing first row of rec + prec 
    else:
        thresh = None
    return thresh
...
def statistic_precision_at_threshold(y_true, y_pred_full, threshold):
    if threshold != None:
        precision_full, recall_full, thresholds_full = precision_recall_curve_modified(y_true, y_pred_full)
        idx_threshold = np.argsort(np.abs(thresholds_full - threshold))[0] # index of thresholds_full for value closest threshold
        precision_at_threshold = precision_full[idx_threshold-1] # precision at corresponding index
    else:
        precision_at_threshold = 0
...
def statistic_recall_at_threshold(y_true, y_pred_full, threshold):
    precision_full, recall_full, thresholds_full = precision_recall_curve_modified(y_true, y_pred_full)
    if threshold<np.max(thresholds_full):
        idx_threshold = np.argsort(np.abs(thresholds_full - threshold))[0] # index of thresholds_full for value closest threshold
        recall_at_threshold = recall_full[idx_threshold-1]
    else:
